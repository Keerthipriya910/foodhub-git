...
export default function Admin() {
  ...
  const fetchRecipes = () => {
    axios
      .get("http://localhost:30022/recipe")
      .then((res) => {
        setRecipes(res.data);
  ...
  const handleSubmit = () => {
    const request = isUpdating
      ? axios.put("http://localhost:30022/update", formData)
      : axios.post("http://localhost:30022/insert", formData);
  ...
  const handleDelete = (rid) => {
    if (window.confirm("Are you sure you want to delete this recipe?")) {
      axios
        .delete(`http://localhost:30022/delete`, { params: { rid } })
        .then((res) => {
          alert(res.data);
          fetchRecipes();
        })
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: "16px", color: "black", fontWeight: "bold" }}>Logout</span>
      </div>
      ...
      <div style={{ display: "flex", width: "100%", flexGrow: 1 }}>
        ...
        >
          ...
          {/* Form */}
          <div style={{ marginBottom: "20px" }}>
            {["rid", "name", "image", "time"].map((field, i) => (
              <input
                key={field}
                type={field === "rid" ? "number" : "text"}
                name={field}
                placeholder={field === "rid" ? "Recipe ID" : `Recipe ${field.charAt(0).toUpperCase() + field.slice(1)}`}
          ...
          {/* Table */}
          <table border="1" cellPadding="10" style={{ width: "100%", marginTop: "20px", color: "black", backgroundColor: "white" }}>
            <thead>
            ...
            <tbody>
              {filteredRecipes.map((recipe) => (
                <tr key={recipe.rid} style={{ color: "black" }}>
                  <td>{recipe.rid}</td>
                  <td>{recipe.name}</td>
                  <td>
